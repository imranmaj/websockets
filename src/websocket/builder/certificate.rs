...
impl Certificate {
    pub fn from_der(der: &[u8]) -> Result<Self, WebSocketError> {
        Ok(Self(NativeTlsCertificate::from_der(der).map_err(|e| WebSocketError::CertificateError(e))?))
    }

    pub fn from_pem(pem: &[u8]) -> Result<Self, WebSocketError> {
        Ok(Self(NativeTlsCertificate::from_pem(pem).map_err(|e| WebSocketError::CertificateError(e))?))
    }

    pub fn to_der(&self) -> Result<Vec<u8>, WebSocketError> {
        self.0.to_der().map_err(|e| WebSocketError::DerConversionError(e))
    }
}
